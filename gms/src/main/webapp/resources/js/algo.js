...
algo.main=(()=>{
	var $wrapper , ctx, script , style, img, compo, json;
	var onCreate = ()=>{
		ctx = $.ctx();
		script = $.script();
		img = $.img();
	...
	var setContentView = ()=>{
		$('#wrapper').html('<h1>ALGORITHM</h1><span id="seq">수 열</span><span id="appl"> 응 용</span>'+
				'<div id="ctn"><table id="tbl" style="width:800px;height:300px; border-collapse: collapse; border: 1px solid black; margin:0 auto">'+
				' <tr style="border: 1px solid black;"> <td id="t__l" style="width: 35%;border: 1px solid black;">'+
				'</td><td id="t__r" style="width: 65%;border: 1px solid black;"></td></tr></table></div>');
		...
		//let anchor = ui.anchor({txt:'등차수열의 하하합'});
		$('<a/>').attr({href : '#'}).html('등차수열').appendTo($('#arith'))			
			.click(e=>{
				$('#ques').empty();					
		...
		$('<a/>').attr({href : '#'}).html('피보나치수열').appendTo($('#arith'))
			.click(e=>{
				$('#ques').empty();
				$('<div />').attr({id:'ques'}).appendTo($t__r);
				$('<h4 />').html('피보나치 수열의 n항부터 m항 까지의 합을 구하시오.').appendTo($('#ques'));
				let arr = [{f:n},{f:m}];
				$(arr).each(function(i){
					$('<label />').html(this.f).appendTo($('#ques'));
					$('<input />').attr({id:this.f ,type : 'text'}).appendTo($('#ques'));
				});
				$('<button/>').addClass('btn btn-primary').attr({type:'button'}).html('결과보기')
					.appendTo($('#ques')).click(e=>{
